...
pub trait DataStreamEngine {
    /// Creates a batch of data client requests that can be
    /// sent to the Aptos data client to progress the stream.
...
impl ContinuousTransactionStreamEngine {
    ...
    fn create_subscription_stream_requests(
        ...
        let max_in_flight_requests = if prefetching_config.enable_dynamic_prefetching {
            // Use the maximum number of in-flight subscription requests
            prefetching_config.max_in_flight_subscription_requests
        } else {
            // Otherwise, use the statically configured maximum number of requests
...
/// Calculates the number of requests to send
fn calculate_num_requests_to_send(
    max_number_of_requests: u64,
    max_in_flight_requests: u64,
    num_in_flight_requests: u64,
) -> u64 {
    // Calculate the number of remaining request slots
    let num_remaining_request_slots = max_in_flight_requests.saturating_sub(num_in_flight_requests);

    // Bound the number of requests to send by the maximum
    min(num_remaining_request_slots, max_number_of_requests)
}
