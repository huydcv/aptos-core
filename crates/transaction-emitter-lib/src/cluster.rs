...
impl Cluster {
    ...
    pub async fn from_host_port(
        ...
        let most_frequent_chain_id =
            get_most_frequent_chain_id_from_instances(instance_states.clone());
        let chain_id: ChainId = match maybe_chain_id {
            Some(c) => c,
            None => {
                warn!(
                    "Chain ID not provided, using the most frequent chain ID from the rest endpoints: {}",
                    most_frequent_chain_id
                );
                most_frequent_chain_id
            },
        };
...
fn get_most_frequent_chain_id_from_instances(instance_states: Vec<(Instance, State)>) -> ChainId {
    // If the chain_id is not provided, we use the most common chain_id from the instances
    // assuming that if there is a mistake in providing the targets, then there is still a majority
    // that has the right chain_id.
...
mod test {
    ...
    #[test]
    fn test_get_most_frequent_chain_id_from_instances() {
        ...
        assert_eq!(
            get_most_frequent_chain_id_from_instances(instance_states),
            chain_id_1
        );
    }
}
